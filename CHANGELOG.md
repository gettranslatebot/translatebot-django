...
- Batching token budget: input and output token limits are now tracked separately via `litellm.get_model_info()`, and the effective output budget is capped at 8K. Previously, models with large advertised output caps (e.g. gpt-5-mini at 128K) let hundreds of strings pack into one batch, triggering degraded structured output — returned-as-source strings, malformed JSON, and missing translations.

### Changed

- Updated aiohttp from 3.13.3 to 3.13.4
- Updated litellm
- Refreshed `uv.lock` with latest compatible versions (charset-normalizer, regex, requests, rich, ruff, tzdata, yarl, and others)
...
- Suppressed harmless Pydantic serialization warnings emitted by litellm during LLM API calls

## [1.0.0] - 2026-03-19

### Changed

- Marked project as stable (Production/Stable)
- Updated litellm from 1.82.2 to 1.82.4
...
- Fixed DeepL `tag_handling="html"` corrupting both format placeholders and real HTML in model field translations. Removed tag handling entirely and switched to `<x>` wrapper tags that DeepL preserves as opaque text.

## [0.9.2] - 2026-03-07

### Fixed

- Fixed source language being included when translating to all configured languages. When no `--target-lang` is specified, the source language (`LANGUAGE_CODE`) and its bare base code are now automatically excluded.
- Fixed DeepL HTML tag handling mode encoding special characters (`<`, `>`, `&`) as HTML entities in translation output. These are now decoded back to their original characters.
...
- PO file extracted comments are now passed to the LLM as translation context, giving the model access to developer hints and notes from source code.

### Fixed

- Fixed DeepL mangling `{placeholder}` style brace placeholders during translation. Placeholders are now wrapped in protective tags before sending to DeepL and restored afterward.
...
[0.9.0]: https://github.com/gettranslatebot/translatebot-django/compare/v0.8.1...v0.9.0
